...
select t.passenger_name, t.passport_no
from ticket t
         join flight f ON t.flight_id = f.id
where t.seat_no = 'B1'
  and f.departure_airport_code = 'MNK'
  and f.arrival_airport_code = 'LDN'
  and f.departure_date::date = '2020-07-28';

select f.flight_no, f.departure_airport_code, f.arrival_airport_code, f.departure_date, f.arrival_date from  flight f
order by f.arrival_date-f.departure_date desc limit 2;

select MAX(f.arrival_date-f.departure_date), MIN(f.arrival_date-f.departure_date), count(*) from flight f
where f.departure_airport_code = 'MNK'
    and f.arrival_airport_code = 'LDN'
   or f.arrival_airport_code = 'MNK'
    and f.departure_airport_code = 'LDN';

select 188-count(*) from ticket t join flight f on f.id = t.flight_id
...
select split_part(t.passenger_name,' ',1),
       ...
       100*count(split_part(t.passenger_name,' ',1))/(select count(t.passenger_name) from ticket t) from ticket t
group by split_part(t.passenger_name,' ',1) order by count(split_part(t.passenger_name,' ',1)) desc;

select t.passenger_name, count(t.passenger_name) from  ticket t
group by t.passenger_name;

select t.flight_id, t.cost, coalesce(t.cost-lead(t.cost) over(order by t.cost desc),0)from ticket t
group by t.cost, t.flight_id
order by cost desc